# Privileged GT-box IDM brake on warmup_test_e2e (ablation, not leaderboard).
...
Write-Host "==== PDM score PrivilegedBrakeAgent warmup_test_e2e ===="
& $py "$root\run_navsim_step.py" "$root\navsim\navsim\planning\script\run_pdm_score.py" `
  train_test_split=warmup_test_e2e `
  worker=sequential `
  agent=privileged_brake_agent `
  experiment_name=privileged_brake_mini `
  metric_cache_path=$env:NAVSIM_EXP_ROOT/metric_cache
if ($LASTEXITCODE -ne 0) { throw "privileged pdm score failed" }
